...
const MiraaiSidebar = ({ logoImage, items = [], onItemClick, activeId }) => {
  ...
  const SidebarContent = ({ isMobile = false }) => (
    <div className={`flex flex-col h-full ${isMobile ? "pt-12" : "pt-20"}`}>
      ...
      {/* Logo Section - Hidden on Mobile, Flex on Desktop (lg) */}
      <div className={`items-center justify-center mb-24 px-8 ${isMobile ? "hidden lg:flex" : "flex"}`}>
        <Link to="/">
          <img
            src={logoImage}
            alt="INAI Verse logo"
            className="w-full md:flex max-w-[160px] md:max-w-[200px] h-auto pointer-events-none select-none"
          />
        </Link>
      </div>
          ...
          >
            <span
              className={`text-[18px] md:text-[20px] font-bold tracking-tight ${
                activeId === item.id ? "opacity-100" : "opacity-100"
              }`}
            >
              {item.label}
            </span>
            <span
              ...
              className={`
                ...
                ${activeId === item.id ? "text-black/60" : "text-black/40 group-hover:text-black"}
              `}
            >
              ›
            </span>

...
import { Bell, Clock, PlayCircle, TrendingUp, Zap, Smile, Trophy, ChevronRight } from "lucide-react";
...
const cardData = [
  ...
  { title: "Prepare Confidently For Competitive Exams", icon: <Trophy size={24} /> },
];
...
const WhyStudentsLove = ({ id }) => {
  ...
  useGSAP(() => {
    ...
    mm.add("(min-width: 1024px)", () => {
      ...
      const getScrollAmount = () => -(scrollContent.scrollWidth - window.innerWidth + 80); 
  ...
  return (
    <section 
      ref={containerRef} 
      id={id} 
      className="w-full bg-black text-white pt-20 pb-30 overflow-hidden"
    >
      ...
      <div className="relative px-6 md:px-6 h-full flex items-center">
        ...
        >
          {cardData.map((card, index) => (
            <div 
              key={index}
              className="love-card flex-shrink-0 w-full lg:w-[350px] h-[300px] md:h-[300px] bg-[#0e0f10] border border-white/10 rounded-[40px] p-10 flex flex-col justify-between relative group hover:border-white/20 transition-all duration-500 shadow-2xl"
            >

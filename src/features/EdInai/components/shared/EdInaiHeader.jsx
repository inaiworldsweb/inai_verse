...
const EdInaiHeader = ({
  ...
  const defaultMenuItems = [
    ...
    {
      label: activeView === "admin" ? "Student View" : "Admin View",
      to: activeView === "admin" ? "/edinai-student" : "/edinai-admin",
    },
  ...
  return (
    <header className="sticky top-0 z-40 bg-black/90 backdrop-blur-md border-b border-white/10">
      ...
      <div className="flex items-center justify-between px-4 py-3 md:px-6 lg:px-8 md:py-4">
        ...
        <nav
          className="flex items-center gap-1.5 md:gap-2 min-w-0 flex-1"
        ...
        <div className="flex items-center gap-2 md:gap-4 flex-shrink-0 ml-2">
          <Link
            to="/"
            className="bg-transparent border-none text-white cursor-pointer p-1.5 md:p-2 hover:text-white/80 hover:bg-white/5 rounded-[10px] transition-colors"
          ...
          <div className="relative" ref={menuRef}>
            ...
            {isMenuOpen && (
              <div className="absolute top-full right-0 mt-2 w-48 bg-[#111] border border-white/10 rounded-[10px] shadow-2xl overflow-hidden py-1 z-50">
                {items.map((item) => (
                  <Link
                    key={item.to}
                    to={item.to}
